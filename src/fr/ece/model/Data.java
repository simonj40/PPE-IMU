...
public class Data {
	...
	public String PATHOUT = "/Users/Simon/Desktop/dataOUT.txt";
	public String PATHIN = "/Users/Simon/Desktop/test1.txt";
	...
	public Coordinates extractAccel(String[] lineArray){
		...
		Coordinates coor = new Coordinates (
				(Double.parseDouble(lineArray[0])/9.81),
				(Double.parseDouble(lineArray[1])/9.81),
				(Double.parseDouble(lineArray[2])/9.81)
				);
	...
	public void generatePos(){
		...
		for(int i=0 ; i<accel.size() ; i++){
			...
			Coordinates c = new Coordinates(
					speed.get(i).x + accel.get(i).x/RATE,
		...
		for(int i=0 ; i<speed.size() ; i++){
			...
			Coordinates c = new Coordinates(
					pos.get(i).x + speed.get(i).x/RATE,
					pos.get(i).y + speed.get(i).y/RATE,
					pos.get(i).z + speed.get(i).z/RATE
						);
			pos.add(c);
		}
	}
	...
	public void correctA() {
		...
		for(int i=0 ; i<accel.size() ; i++){
			...
			accel.get(i).x = 2*(Q1*Q3 + Q0*Q2)*Ax + 2*(Q2*Q3 -Q0*Q1)*Ay + (1-2*Math.pow(Q1, 2)-2*Math.pow(Q2, 2))*Az;	
	...
	public void removeG() {
		...
		for(int i=0 ; i<accel.size() ; i++){
			...
			accel.get(i).x -= -2*(Q1*Q3 - Q0*Q2);
			accel.get(i).y -= -2*(Q2*Q3 + Q0*Q1);
			accel.get(i).x -= -(1-2*Math.pow(Q1, 2)-2*Math.pow(Q2, 2));			
		}
